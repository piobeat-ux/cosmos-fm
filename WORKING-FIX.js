...
// 1. Исправляем DataContext.tsx - правильные имена + fallback
    ...
    const loadData = async () => {
      try {
        ...
        timeoutId = setTimeout(() => {
          if (!cancelled) {
            console.warn('⚠️ Таймаут (15 сек), используем fallback данные');
            setLoading(false);
          }
        }, 15000);
...
// 3. Исправляем App.tsx - добавляем error
console.log('3/4 Исправление App.tsx...');

if (fs.existsSync('src/App.tsx')) {
  let content = fs.readFileSync('src/App.tsx', 'utf-8');
  
  // Добавляем error в useData
...
// 4. Увеличиваем таймаут в supabase.ts
console.log('4/4 Увеличение таймаута...');
...
if (fs.existsSync('src/lib/supabase.ts')) {
  ...
  console.log('✅ supabase.ts - таймаут 15 сек');
}
...
console.log('2. ✅ DataContext - добавлены FALLBACK данные');
console.log('3. ✅ Header - используется navigation');
console.log('4. ✅ App - добавлен error из useData()');
console.log('\n🚀 ЗАПУСТИТЕ:');
console.log('  npm run dev');
console.log('\n✅ РЕЗУЛЬТАТ:');
console.log('  - Сайт ЗАГРУЗИТСЯ с fallback данными!');
console.log('  - Навигация работает');
console.log('  - Все страницы доступны');
console.log('  - Если Supabase станет доступен - данные обновятся');
console.log('='.repeat(70));
