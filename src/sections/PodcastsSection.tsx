...
export function PodcastsSection() {
  ...
  const categories: string[] = ['Все', ...Array.from(new Set(podcasts.map((p: any) => p.category).filter(Boolean)))];
  ...
  return (
    <section id="podcasts" className="py-20 fade-in">
      <div className="max-w-6xl mx-auto section-padding">
        ...
        <div className="mb-6">
          <SearchBar value={searchQuery} onChange={setSearchQuery} placeholder="Поиск подкастов..." />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPodcasts.length === 0 ? (
            ...
            filteredPodcasts.map((podcast: any, index: number) => {
              ...
              return (
                <div key={podcast.id} className="glass-card rounded-2xl p-6 hover:border-[#6366f1]/50 transition-all duration-300 slide-up" style={{ animationDelay: `${index * 0.05}s` }}>
                  <div className="relative mb-4">
                    ...
                    {podcast.audio_url && (
                      <button onClick={() => handlePlayPodcast(podcast)} className={`absolute inset-0 flex items-center justify-center bg-black/60 rounded-xl transition-opacity ${playing ? 'opacity-100' : 'opacity-0 hover:opacity-100'}`}>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-[#27273a]">
                    ...
                    <button onClick={() => handleLike(podcast)} disabled={liked} className={`p-2 transition-colors ${liked ? 'text-[#ef4444]' : 'text-[#71717a] hover:text-[#ef4444]'}`}>
