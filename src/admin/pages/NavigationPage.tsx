          ...
          <div key={link.id} className="glass-card rounded-xl p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-[#71717a]">{link.url}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#13131f] rounded-2xl border border-[#27273a] p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <input placeholder="Название *" value={formData.label} onChange={e => setFormData({...formData, label: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-[#0a0a0f] border border-[#27273a] focus:border-[#6366f1] focus:outline-none" />
              <input placeholder="URL (например: #/schedule)" value={formData.url} onChange={e => setFormData({...formData, url: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-[#0a0a0f] border border-[#27273a] focus:border-[#6366f1] focus:outline-none" />
              <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-[#0a0a0f] border border-[#27273a] focus:border-[#6366f1] focus:outline-none">
                <option value="anchor">Якорь</option>
                <option value="external">Внешняя ссылка</option>
              </select>
              <input type="number" placeholder="Порядок" value={formData.order_index} onChange={e => setFormData({...formData, order_index: parseInt(e.target.value) || 1})} className="w-full px-4 py-2 rounded-xl bg-[#0a0a0f] border border-[#27273a] focus:border-[#6366f1] focus:outline-none" />
