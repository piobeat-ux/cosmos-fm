  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {hosts.length === 0 ? <p className="col-span-full text-center py-12 text-[#4A6578]">Нет ведущих</p> : hosts.map(h => (
          <div key={h.id} className="glass-card rounded-xl p-4">
            <div className="flex items-start gap-3">
              {h.photo_url ? <img src={h.photo_url} className="w-16 h-16 rounded-full object-cover border-2 border-[#28B9D040]" /> : <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${h.color||COLORS[0]} flex items-center justify-center text-[#1A2B3C] font-bold`}>{h.initials||h.name.split(' ').map(n=>n[0]).join('').substring(0,2)}</div>}
              <div className="flex-1 min-w-0">
                <h3 className="font-bold truncate">{h.name}</h3>
                <p className="text-sm text-[#6366f1]">{h.role||'Ведущий'}</p>
                <p className="text-xs text-[#4A6578] mt-1">{h.hotel||'Отель не указан'}</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#F5FBFD] rounded-2xl border border-[#28B9D040] p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <input placeholder="Имя *" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-white border border-[#28B9D040] focus:border-[#6366f1] focus:outline-none" />
