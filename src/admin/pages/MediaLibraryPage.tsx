...
export function MediaLibraryPage({ kind }: { kind: 'show' | 'podcast' }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      const { error: saveError } = await supabase.rpc('save_media_record', {
        ...
        record_data: { ...form, title: form.title.trim(), audio_url: form.asset_id ? '' : form.audio_url, catalog_mode: scheduled ? 'after_airing' : form.catalog_mode },
  ...
  const remove = async (record: Show | Podcast) => {
    ...
    setBusy(true); setError('');
    try {
      const { error: deleteError } = await supabase.from(kind === 'show' ? 'shows' : 'podcasts').delete().eq('id', record.id);
  ...
  return <div className="space-y-5">
    ...
    {open && <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 p-4 flex items-start justify-center">
      <form onSubmit={save} role="dialog" aria-modal="true" aria-labelledby="media-title" className="my-8 w-full max-w-2xl rounded-2xl bg-white p-6 space-y-4">
        ...
        <ImageUpload value={form.cover_url} onChange={cover_url => setForm(current => ({ ...current, cover_url }))} label="Обложка" />
        <AudioUpload assetId={form.asset_id} legacyUrl={form.audio_url} onChange={asset => setForm(current => ({ ...current, asset_id: asset.id, audio_url: '', duration: `${Math.floor(asset.duration_seconds / 60)} мин ${asset.duration_seconds % 60} с` }))} />
        ...
        {scheduled ? <div className="rounded-xl bg-[#F5FBFD] p-4 space-y-3"><label className="block">Дата и время по Москве<input type="datetime-local" required value={start} onChange={event => setStart(event.target.value)} className="mt-1 w-full rounded-lg border p-2" /></label><label className="flex gap-2"><input type="checkbox" checked={weekly} onChange={event => setWeekly(event.target.checked)} /> Повторять каждую неделю</label><p className="text-sm text-[#4A6578]">Запись появится в каталоге после первого эфира. При сохранении черновика эфир тоже останется черновиком.</p></div>
        ...
        <div className="flex gap-3"><button type="submit" disabled={busy} className="btn-primary">{busy ? 'Сохранение…' : 'Сохранить'}</button><button type="button" onClick={() => setOpen(false)} disabled={busy} className="btn-secondary">Отмена</button></div>
      </form>
    </div>}
  </div>;
}
