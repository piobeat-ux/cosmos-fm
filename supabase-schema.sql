...
-- Профили: только чтение для себя
CREATE POLICY "Users can view own profile" ON public.profiles
  FOR SELECT USING (auth.uid() = id);

-- Контент: все читают, только админы пишут
CREATE POLICY "Public read access" ON public.shows FOR SELECT USING (true);
CREATE POLICY "Admin write access" ON public.shows 
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY "Public read access" ON public.hosts FOR SELECT USING (true);
CREATE POLICY "Admin write access" ON public.hosts 
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Public read access" ON public.podcasts FOR SELECT USING (true);
CREATE POLICY "Admin write access" ON public.podcasts 
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admin write access" ON public.site_settings 
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role = 'admin'
